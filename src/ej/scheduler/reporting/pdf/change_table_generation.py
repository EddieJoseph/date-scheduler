...
from ej.scheduler.util.date_utils import convert_to_date
from ej.scheduler.util.file_generation_utils import translate_umlauts, remove_numbers_at_end
from ej.scheduler.util.row_names import RowNames


def has_changed(date_1, date_2):
    return not text_equals(date_1, date_2, RowNames.NAME) or not equals(date_1, date_2, RowNames.DATE) or not equals(
        date_1, date_2, RowNames.TIME) or not equals(date_1, date_2, RowNames.CALLED_UP) or not text_equals(date_1,
                                                                                                            date_2,
                                                                                                            RowNames.THEME) or not text_equals(
        date_1, date_2, RowNames.RESPONSIBLE)
...
def generate_change_table_tex(data, version, old_data, old_versions, year):
    ...
    while i < len(old_data):
        ...
        change_table_tex += load_change_table_start().replace('$version_old', previous_version).replace('$version_new',
                                                                                                        current_version)
            ...
            for event in changed_events_on_day.iterrows():
                ...
                if prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                    ...
                    e_time = change_text(translate_umlauts(
                        prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                            RowNames.TIME.value]), translate_umlauts(event[1][RowNames.TIME.value]))
                if prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                    RowNames.NAME.value] != remove_numbers_at_end(event[1][RowNames.NAME.value]):
                    e_name = change_text(translate_umlauts(
                        prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                            RowNames.NAME.value]),
                        remove_numbers_at_end(translate_umlauts(event[1][RowNames.NAME.value])))
                if prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                    RowNames.CALLED_UP.value] != event[1][RowNames.CALLED_UP.value]:
                    e_group = change_text(translate_umlauts(
                        prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                            RowNames.CALLED_UP.value]), translate_umlauts(event[1][RowNames.CALLED_UP.value]))
                if prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                    RowNames.THEME.value] != event[1][RowNames.THEME.value]:
                    e_theme = change_text(translate_umlauts(
                        prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                            RowNames.THEME.value]), translate_umlauts(event[1][RowNames.THEME.value]))
                if prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                    RowNames.RESPONSIBLE.value] != event[1][RowNames.RESPONSIBLE.value]:
                    e_responsible = change_text(translate_umlauts(
                        prevoius.loc[prevoius[RowNames.ID.value] == event[1][RowNames.ID.value]].iloc[0][
                            RowNames.RESPONSIBLE.value]), translate_umlauts(event[1][RowNames.RESPONSIBLE.value]))
                change_table_tex += generate_change_table_row(e_date, e_time, e_name, e_group, e_theme, e_responsible)
